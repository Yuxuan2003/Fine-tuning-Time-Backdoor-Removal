...
combined_model2.load_state_dict(torch.load("/data2/zyx/DRUPE-main/DRUPE-main/finetune_after_clp/drupe_cifar10_gtsrb/drupe_cifar10_gtsrb_differenttrigger_sametarget_1.pth"))
...
test_posion_images = [img for img, label in test_posion_data if label != 12]
...
labels = torch.tensor([12]*len(test_posion_images),dtype=torch.long)
...
label_12_images = [(img, target) for img, target in train_posion_data if target == 12]
non_label_12_images = [(img, target) for img, target in train_posion_data if target != 12]
...
filename = f'/data2/zyx/DRUPE-main/DRUPE-main/loss_list/drupe_gtsrb_diftrigger_sametarget_{number}.pth'
